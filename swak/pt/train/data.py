...
class TestDataBase(ABC):
    ...
    def sample(self, size: int) -> Batch:
        ...
        -------
        tuple
            A tuple of input tensors to call your model with. Even if your
            model only takes a single tensor as input, this must be a tuple
            to cover the general case of your model taking more than one
            tensor as input. If this is not the case, simple return a 1-tuple!
        Tensor
...
class TrainDataBase(TestDataBase):
    ...
    def __call__(self, batch_size: int) -> Batches:
        ...
        Iterator
            ...
            with. This must be a tuple even if your model only takes a single
            tensor as input to cover the general case of your model taking more
            than one tensor as input. If this is not the case, simple make it a
            1-tuple! The second elements of the tuple is a single tensor with
