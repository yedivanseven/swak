...
class TestFlatUsage(unittest.TestCase):
    ...
    def setUp(self):
        self.m = ThreadMap(to_tuple, None, True)
    ...
    def test_empty_tuple_result_disappears(self):
        m = ThreadMap(to_empty_tuple, None, True)
        actual = m([1, 2, 3])
        self.assertListEqual([], actual)

    def test_single_element_tuple_unpacked(self):
        m = ThreadMap(to_single_tuple, None, True)
        actual = m([1, 2, 3])
        self.assertListEqual([3, 4, 5], actual)
    ...
    def test_mixed_tuple_and_scalar(self):
        m = ThreadMap(to_tuple_or_scalar, None, True)
        actual = m([1, 2, 3])
...
class TestThreadPoolAttributes(unittest.TestCase):
    ...
    def test_instantiation(self):
        _ = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2))

    def test_has_transform(self):
        m = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2))
        self.assertTrue(hasattr(m, 'transform'))
...
class TestMapAttributes(unittest.TestCase):
    ...
    def test_has_timeout_arg(self):
        m = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2), 42)
        self.assertTrue(hasattr(m, 'timeout'))

    def test_timeout_arg_correct(self):
        m = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2), Cls)
...
class TestMisc(unittest.TestCase):
    ...
    def test_wrapper_pickle_raises_lambda(self):
        m = ThreadMap(plus_2, lambda x: tuple(x))  #noqa
        with self.assertRaises(AttributeError):
            _ = pickle.loads(pickle.dumps(m))

    def test_pickle_raises_with_lambda(self):
        m = ThreadMap(lambda x: x + 2, lambda x: tuple(x))  #noqa
        with self.assertRaises(AttributeError):
            _ = pickle.loads(pickle.dumps(m))

    def test_default_lambda_repr(self):
        m = ThreadMap(lambda x: x > 3)
        expected = "ThreadMap(lambda, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_function_repr(self):
        m = ThreadMap(plus_2)
        expected = "ThreadMap(plus_2, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_class_repr(self):
        m = ThreadMap(Cls)
        expected = "ThreadMap(Cls, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_obj_repr(self):
        m = ThreadMap(Call())
        expected = "ThreadMap(Call(...), None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_classmethod_repr(self):
        m = ThreadMap(Cls.c)
        expected = "ThreadMap(Cls.c, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_staticmethod_repr(self):
        m = ThreadMap(Cls().s)
        expected = "ThreadMap(Cls.s, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_method_repr(self):
        m = ThreadMap(Cls().m)
        expected = "ThreadMap(Cls.m, None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_default_argrepr(self):
        m = ThreadMap(A(1))
        excepted = "ThreadMap(A(1), None, False, 16, '', None, (), None)"
        self.assertEqual(excepted, repr(m))

    def test_default_indentrepr(self):
        m = ThreadMap(Ind([1, 2, 3]))
        expected = "ThreadMap(Ind()[3], None, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_wrapper_repr(self):
        m = ThreadMap(plus_2, tuple)
        expected = "ThreadMap(plus_2, tuple, False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_wrapper_argrepr(self):
        m = ThreadMap(plus_2, A(1))
        expected = "ThreadMap(plus_2, A(1), False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_wrapper_indentrepr(self):
        m = ThreadMap(plus_2, Ind([1, 2, 3]))
        expected = "ThreadMap(plus_2, Ind()[3], False, 16, '', None, (), None)"
        self.assertEqual(expected, repr(m))

    def test_threadpool_repr(self):
        m = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2))
        expect = "ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2), None)"
        self.assertEqual(expect, repr(m))

    def test_map_arg_repr(self):
        m = ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2), 42)
        expected = "ThreadMap(plus_2, None, False, 8, 'tm', plus, (1, 2), 42)"
        self.assertEqual(expected, repr(m))

    def test_map_kwarg_repr(self):
        m = ThreadMap(plus_2, timeout=42)
        expected = "ThreadMap(plus_2, None, False, 16, '', None, (), 42)"
        self.assertEqual(expected, repr(m))
