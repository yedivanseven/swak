...
from swak.pd import RollingWindow


class TestDefaultAttributes(unittest.TestCase):

    def setUp(self):
        self.roll = RollingWindow()
...
class TestAttributes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.roll = RollingWindow(*self.args, **self.kwargs)
...
class TestMisc(unittest.TestCase):
    ...
    def test_default_repr(self):
        roll = RollingWindow()
        expected = 'RollingWindow()'
        self.assertEqual(expected, repr(roll))

    def test_custom_repr(self):
        roll = RollingWindow('foo', 1, answer=42)
        expected = "RollingWindow('foo', 1, answer=42)"
        self.assertEqual(expected, repr(roll))
