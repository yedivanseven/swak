...
from swak.pt.blocks import ActivatedGatedBlock
...
class TestDefaultAttributes(unittest.TestCase):
    ...
    def setUp(self):
        self.block = ActivatedGatedBlock(4)
    ...
    def test_reset_parameters_called_on_instantiation(self, linear):
        ...
        with patch('torch.nn.PReLU.reset_parameters') as mock:
            _ = ActivatedGatedBlock(4, activate, gate)
    ...
    def test_reset_parameters_called_on_activations(self):
        block = ActivatedGatedBlock(4, ptn.PReLU(), ptn.PReLU())
    ...
    def test_call_new(self):
        new = self.block.new()
        self.assertIsInstance(new, ActivatedGatedBlock)
...
class TestUsage(unittest.TestCase):
    ...
    def setUp(self):
        self.block = ActivatedGatedBlock(4, identity, identity, bias=False)
        self.block.widen.weight.data = pt.ones(16, 4)
        self.block.shrink.weight.data = pt.ones(8, 16)
    ...
    def test_1d(self):
        ...
        expected = pt.ones(4) * 4096
        pt.testing.assert_close(actual, expected)

    def test_2d(self):
        inp = pt.ones(3, 4)
        actual = self.block(inp)
        expected = pt.ones(3, 4) * 4096
        pt.testing.assert_close(actual, expected)

    def test_3d(self):
        inp = pt.ones(2, 3, 4)
        actual = self.block(inp)
        expected = pt.ones(2, 3, 4) * 4096
        pt.testing.assert_close(actual, expected)

    def test_4d(self):
        inp = pt.ones(5, 2, 3, 4)
        actual = self.block(inp)
        expected = pt.ones(5, 2, 3, 4) * 4096
        pt.testing.assert_close(actual, expected)

    def test_empty_dims(self):
        inp = pt.ones(3, 0, 4)
        actual = self.block(inp)
        expected = pt.ones(3, 0, 4) * 4096
        pt.testing.assert_close(actual, expected)
