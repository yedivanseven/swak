...
import torch as pt
from swak.pt.mix import ArgsWeightedSumMixer


class TestAttributes(unittest.TestCase):

    def setUp(self):
        self.mix = ArgsWeightedSumMixer(3)
    ...
    def test_call_new_default(self):
        new = self.mix.new()
        self.assertIsInstance(new, ArgsWeightedSumMixer)
...
class TestUsage(unittest.TestCase):
    ...
    def setUp(self):
        self.mix = ArgsWeightedSumMixer(2)
        self.f1 = pt.tensor([2., 3., 5., 7.])
        self.f2 = pt.tensor([11., 13., 17., 19.])
        self.expected = 0.5 * (self.f1 + self.f2)

    def test_1d(self):
        actual = self.mix(self.f1, self.f2)
        pt.testing.assert_close(actual, self.expected)

    def test_2d(self):
        actual = self.mix(self.f1.tile(3, 1), self.f2.tile(3, 1))
        pt.testing.assert_close(actual, self.expected.tile(3, 1))

    def test_3d(self):
        actual = self.mix(self.f1.tile(5, 3, 1), self.f2.tile(5, 3, 1))
        pt.testing.assert_close(actual, self.expected.tile(5, 3, 1))

    def test_4d(self):
        actual = self.mix(self.f1.tile(7, 5, 3, 1), self.f2.tile(7, 5, 3, 1))
        pt.testing.assert_close(actual, self.expected.tile(7, 5, 3, 1))

    def test_empty_dims(self):
        actual = self.mix(self.f1.tile(5, 0, 1), self.f2.tile(5, 0, 1))
